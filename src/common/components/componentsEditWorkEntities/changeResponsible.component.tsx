...
export const ChangeResponsibleComponent = (props: Props) => {
  ...
  const resetForm = () => {
    setLoad(false), setError(false), setData([]), reset();
  };
    ...
    try {
      ...
      const usersData = data.map((user) => {
        return {
          ...
          email: user?.email,
          numberContact1: user?.numberContact1,
          userId: user?.id,
        };
      });
      setLoad(false);
  ...
  return (
    <>
      <Button className="rounded-full !h-10" style={{ padding: "1.25rem" }} onClick={() => setVisible(true)}>
        Cambiar responsable
      </Button>

      <Dialog
        header={<h1>Cambiar responsable</h1>}
          ...
          {data.length > 0 ? (
            <>
              <Card className="card">
                ...
                <div className="overflow-hidden max-w-[calc(100vw-4.6rem)] sm:max-w-[calc(100vw-10.1rem)] lg:max-w-[calc(100vw-27.75rem)] block md:block borderless reverse-striped">
                  <DataTable
                    value={data}
                    paginator
                ...
                <div className="flex justify-center mt-8">
                  <Button
                    ...
                    label="Cancelar"
                  ></Button>
                  <Button className="rounded-full !h-10" onClick={() => setChangeUser(true)} disabled={changeBtn}>
                    Cambiar
                  </Button>
                </div>
              </Card>
