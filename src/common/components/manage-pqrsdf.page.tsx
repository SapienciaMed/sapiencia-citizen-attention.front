...
const ManagePqrsdf = () => {
  ...
  const getPqrsdf = async (param: IrequestPqrsdf) => {
    ...
    const pqrsdfData = data.map((pqr: IpqrsdfByReques) => {
      return {
        ...
        dias:
          pqr["OBS_TIPO_DIAS"] === "Calendario"
            ? countDaysCalendar(pqr["PQR_FECHA_CREACION"])
            : countDays(pqr["PQR_FECHA_CREACION"], workingDays),
  ...
  function focusBtn(id: string) {
    ...
    document.querySelectorAll('.btn-t').forEach((btn: HTMLElement)=>{
      if (id==btn.id) {
        btn.style.color = "#533893";
        btn.style.borderBottom = "solid 2px #533893";
      }else{
      ...
      <div className="container-div">
        <Card title={<p className="text-3xl block pb-5">{tittle}</p>} className="card-container card-top">
          ...
          <div className="div-end mt-10 mb-10">
            <button className="btn-t btn-1" id="btn-1" onClick={() => focusBtn("btn-1")}>
              {tittleButton}
            </button>
            <button className="btn-t btn-2" id="btn-2" onClick={() => focusBtn("btn-2")}>
              Solicitudes cerradas
            </button>
            <button className="btn-t btn-3" id="btn-3" onClick={() => focusBtn("btn-3")}>
